...
from Info.SingleAccount import main_client, address, private_key
from Info.Contracts import eth_contract_address
...
async def starknet_run():
    for i in range(1, 3):
        ...
        print(f"第{i}个钱包交易总预计费用: {gas_fee} ETH")

        # 开始执行所有合约
        print(f"第{i}个钱包正在执行所有合约...")
